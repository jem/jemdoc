...
"""jemdoc version 0.6.5, 2008-11-19."""
...
def br(b, f, tableblock=False):
    ...
    if tableblock:
        ...
        f.tablerow += 1
        f.tablecol = 1
        b = re.sub(r, r'</td></tr>\n<tr class="r%d"><td>' % f.tablerow, b)

        # jemjemjemjem
        # Deal with |, meaning </td><td>
        r = re.compile(r"(?<!\\)\|", re.M + re.S)
        b = re.sub(r, r'</td><td class="c%d">' % f.tablecol, b)
        #while re.search(r, b):
            #f.tablecol += 1
        #b = re.subn(r, r'</td><td class="c%d">' % f.tablecol, b, 1)
...
def gethl(lang):
    ...
    if lang in ('py', 'python'):
        ...
        d['statement'] = putbsbs(['if', 'else', 'printf', 'return'])
        d['builtin'] = putbsbs(['static', 'typedef', 'int', 'float', 'double',
                                'void', 'clock_t', 'struct', 'long'])
        d['special'] = (['#include.*', '#define'])
        d['error'] = putbsbs(['\w*Error',])
        d['commentuntilend'] = ['//', '/*', ' * ', ' */']
        d['strings'] = True
    elif lang == 'sh':
        d['statement'] = putbsbs(['cd', 'ls', 'sudo', 'cat', 'alias', 'grep',
                                  'killall', 'find', 'sed', 'gs',
                                  'tee', 'gzip'])
        ...
        # look for blocks.
            ...
            if infoblock:
                ...
                if len(g) in (0, 1): # info block.
                    ...
                    if len(g) >= 3 and g[2]:
                        name += ' id="%s"' % g[2]
                    out(f.outf, '<table%s><tr class="r1"><td>\n' % name)
                    f.tablerow = 1
                    f.tablecol = 1
